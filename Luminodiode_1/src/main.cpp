#include <Arduino.h> // Библиотека Arduino.h для работы с платами(например, ESP8266)

int Pin = 2;  /* Переменная Pin отвечает за сам пин, который определяется при помощи библиотеки pins_arduino.h.
